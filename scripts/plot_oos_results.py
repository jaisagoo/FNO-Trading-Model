...
judging a strategy on a *true* out-of-sample (OOS) window:
...
Usage
-----
    python scripts/plot_oos_results.py                      # default tickers, live data
    python scripts/plot_oos_results.py --offline            # use cached parquet only
    python scripts/plot_oos_results.py --tickers ES=F NQ=F CL=F --offline
"""
...
def main() -> None:
    ...
    log.info("wrote %d figures to %s", len(written), outdir)
    for p in written:
        log.info("  %s", p.name)
